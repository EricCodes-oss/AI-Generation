...
- “父母养老与照护压力”不进入 V1。
- 每个重要生产环节均由对应业务 Skill 返回可审阅成果并等待用户确认；选题、脚本、成片是不可跳过的关键审批点。
...
**交付物：** 先实现带用户门控的每日热点检索（30–40 条有效来源、5–8 条评论深挖、三层时间窗口和检索报告），再实现手工导入/浏览器采样适配器、来源归档、内容去重、三大栏目分类、风险初筛、可解释评分和 Top 3 报告。

**设计依据：** `docs/superpowers/specs/2026-08-04-skill-gated-content-workflow-design.md`。

**计划文件：** 书面设计复核后创建 `2026-08-04-phase-2-topic-research-ranking.md`。
