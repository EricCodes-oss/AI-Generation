...
def normalize_source(raw: RawCollectionItem, context: NormalizationContext) -> ResearchSource:
    ...
    warnings: list[str] = []
    published_at = _parse_published_at(_value(payload, _FIELD_ALIASES["published_at"]), warnings)
...
def normalize_batch(batch: CollectionBatch, context: NormalizationContext) -> NormalizationResult:
...
def _resolve_platform(fallback: ResearchPlatform, payload: dict[str, Any]) -> ResearchPlatform:
...
def _confidence_for_source(content_id: str | None, canonical_url: str | None) -> ConfidenceLevel:
